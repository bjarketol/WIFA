...
def validate_system_yaml(base_path, case_path, windio_path, schema_path):
    """Helper function to validate a system.yaml file"""
    system_yaml = base_path / case_path / "wind_energy_system/system.yaml"
...
def test_windio_4turbines_basic_schema(base_path, windio_path, schema_path):
    ...
    system_files = ['system.yaml', 'system_noFlowField.yaml', 'system_noXYgrid.yaml', 'system_runAll.yaml', 'system_zplaneList.yaml']
    for system in system_files:
        no_field_yaml = base_path / Path(f"windio_4turbines/wind_energy_system/{system}")
...
def test_windio_4turbines_abl_stable_schema(base_path, windio_path, schema_path):
    ...
    validate_system_yaml(base_path, "windio_4turbines_ABL_stable", windio_path, schema_path)

def test_windio_4turbines_multiple_schema(base_path, windio_path, schema_path):
    """Test 4 turbines multiple turbines case schema"""
    validate_system_yaml(base_path, "windio_4turbines_multipleTurbines", windio_path, schema_path)
...
def test_windio_4turbines_profiles_stable_schema(base_path, windio_path, schema_path):
    ...
    base_yaml = base_path / "windio_4turbines_profiles_stable/wind_energy_system/system.yaml"
    validate_yaml(base_yaml, windio_path / schema_path)
    
    # Test grid variant
    grid_yaml = base_path / "windio_4turbines_profiles_stable/wind_energy_system/system_grid.yaml"
...
if __name__ == '__main__':
    ...
    try:
        ...
        test_windio_4turbines_profiles_stable_schema(base_path, windio_path, schema_path)
