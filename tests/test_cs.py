...
def _run_cs(wes_dir, output_dir):
    ...
    for yaml_input in wes_dir.glob("system*.yaml"):
        ...
        run_code_saturne(yaml_input, test_mode=False, output_dir=output_dir+"_"+str(i))
        i+=1
...
def test_cs_abl_stable():
    wes_dir = test_path / '../examples/cases/windio_4turbines_ABL_stable/wind_energy_system/'
    _run_cs(wes_dir, "abl_stable")

def test_cs_profiles():
    wes_dir = test_path / '../examples/cases/windio_4turbines_profiles_stable/wind_energy_system/'
