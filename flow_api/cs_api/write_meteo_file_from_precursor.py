...
if __name__ == "__main__":
    ...
    rotation_angle = np.radians(270.0-hub_dir) - np.arctan2(v[idx_zalpha], u[idx_zalpha])
    ...
    for i in range(len(z)):
        meteo_file.write(" "+str(z[i])+delimiter+str(T[i])+delimiter+str(0.0)+delimiter+str(0.0)+"\n")
    # last line extrapolation to have h>1500m
    meteo_file.write(" "+str(z[-1]+10)+delimiter+str(T[-1])+delimiter+str(0.0)+delimiter+str(0.0)+"\n")

    meteo_file.write("/ Wind profile: number of altitudes,(alt.,u,v,ect,eps)\n")
    meteo_file.write(" "+str(len(z)+1)+"\n")
    for i in range(len(z)):
        meteo_file.write(" "+str(z[i])+delimiter+str(u_rot[i])+delimiter+str(v_rot[i])+delimiter+str(k[i])+delimiter+str(eps[i])+"\n")
    # last line extrapolation to have h>1500m
    meteo_file.write(" "+str(z[-1]+10)+delimiter+str(u_rot[-1])+delimiter+str(v_rot[-1])+delimiter+str(k[-1])+delimiter+str(eps[-1])+"\n")
    #
    meteo_file.close()
