...
def phim_u_nieuwstadt(z, zi, dlmo, alpha):
    ...
    coef = (phim_hogstrom_s(z, dlmo)/z)*pow(max(1-z/zi, 0), a)*np.cos(b*np.log(max(1-z/zi, 1e-12)))
    return coef
...
def phim_v_nieuwstadt(z, zi, dlmo, alpha):
    a = 0.5*alpha-1
    b = 0.5*pow(alpha*(alpha+2), 0.5)
    coef = (phim_hogstrom_s(z, dlmo)/z)*pow(max(1-z/zi, 0), a)*np.sin(b*np.log(max(1-z/zi, 1e-12)))
    return coef
...
def tke_nieuwstadt(z, zi, dlmo, alpha):
    ...
    tke = 0.5*np.sqrt(d*(phim_hogstrom_s(z, dlmo) - zeta)/(phih_hogstrom_s(z, dlmo)*(w0+w1*phim_hogstrom_s(z, dlmo)/(phim_hogstrom_s(z, dlmo)-zeta)-(1-a1)*zeta/(ctheta*(phim_hogstrom_s(z, dlmo)-zeta)))))*pow(1-z/zi, alpha/2+1)
    return tke
