...
class NoCurlyBracesDumper(yaml.SafeDumper):
    def increase_indent(self, flow=False, indentless=False):
        return super(NoCurlyBracesDumper, self).increase_indent(flow=False, indentless=False)
...
def extract_array_from_text(text, array_name):
    ...
    array_data = [[float(num) for num in row.split(',')] for row in array_data.split('],[')]
...
def process_turbine_file(file_path):
    ...
    yaml_string = yaml.dump(turbine_data, Dumper=NoCurlyBracesDumper, sort_keys=False, default_flow_style=None)
    ...
    with open(output_file, 'w') as file:
        yaml.dump(turbine_data, file, Dumper=NoCurlyBracesDumper, sort_keys=False, default_flow_style=None)
